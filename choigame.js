...
String.prototype.isPositiveNumber = function() {
    return this.match(/\d+/)?.[0].length === this.length;
}
let global_scope = {},
outside_depent = [],
devc = 0;
...
onChangeType = () => {
    ["s",
        "sb"].includes(document.forms.generator.pack_type.value) ? document.getElementById("entrypoint").innerHTML = `
    <br><br>
...
updateDepent = () => {
    ...
    if ([dver1, dver2, dver3, duuid].some(v => v.value === "")) return alert("You need to ne fill the blank");
    if (!duuid.value.isV4UUID()) return alert("Invalid UUID");
    if (!dver1.value.isPositiveNumber() || !dver2.value.isPositiveNumber() || !dver3.value.isPositiveNumber()) return alert("Invalid version number");
...
onSubmit = () => {
    ...
    output.innerHTML = `<pre><code class="language-json">${JSON.stringify(data, {
        type: "text/json"
    }, "\t")}</code></pre>`;
    ...
    download.innerHTML = `
    <br>
    <button type="button" class="button_block" onclick="return coppyToClipboard();" class="button_block1">Coppy to clipboard</button>
    <br>
    <button type="button" class="button_block" onclick="return download();" class="button_block1">Download file</button>
    `;
    global_scope = data;
},
download = (data) => {
...
saveJsonFile = (name, data) => {
    ...
    link.dataset.downloadurl = ["text/json",
        link.download,
        link.href].join(":");
